...
"""
Validate a trained YOLOv8 detection model on a detection dataset

Usage:
    $ python val.py --weights yolov8s.pt --data coco128.yaml --img 640
...
def run(
    ...
    if training:  # called by train.py
        ...
        # Load model
        model = YOLO(weights)
        model.val(data=data, save_json=save_json, save_hybrid=save_hybrid, conf=conf, iou=iou, max_det=max_det, half=half,
                  dnn=dnn, plots=plots)
    ...
    for i, c in enumerate(ap_class):
        maps[c] = ap[i]
    logging.info(f"mp: {mp}, mr: {mr}, map50: {map50}, map: {map}")
...
def parse_opt():
    ...
    parser.add_argument('--weights', nargs='+', type=str, default=ROOT / 'weights/yolov8n.pt', help='model path(s)')
    ...
    parser.add_argument('--plots', default= True, action='store_true', help='')
...
def main(opt):
    ...
    if opt.task in ('train', 'val', 'test'):  # run normally
        if opt.conf > 0.001:  # https://github.com/ultralytics/yolov5/issues/1466
